...
import { Answer, GetProjectDetailExample, Post, ProjectJSON } from "./projectType";
    ...
    for (const url of urls){
        try {
            ...
            if (posts.length !== 0) {
                ...
                // sort trust level
                posts.sort((a, b) => {
                    const actionCountA = (a.actions_summary.find(action => action.id === 2) || { count: 0 }).count;
                    const actionCountB = (b.actions_summary.find(action => action.id === 2) || { count: 0 }).count;
                ...
                for  (const post of posts){
                    ...
                    if (post.cooked !== "") {
                        let ans :Answer = {
                            ...
                            staff: post.staff,
                            like_count: post.actions_summary.find(action => action.id === 2)?.count || 0,
                        }
                ...
                fs.writeFileSync(filePath, JSON.stringify(projectOwner, null, 2), 'utf-8');
            }
